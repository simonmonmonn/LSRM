        ...
        with autocast():
            if vis_weight is not None:
                ...
                if masktype == 0:
                    ...
                    x = F.silu(x @ (vis_weight[0] + vis_weight[1]).permute(0, 2, 1))
                    score, _ = x.sort(dim=2)
                    threshold = score[:, :, int(self.delta * score.size(2))].unsqueeze(2)
                    mask_change = torch.ones_like(x).half()
                    mask_change[torch.where(x < threshold.expand_as(x))] = self.lambd
                    vis_mask = vis_mask * mask_change
                    x = x * vis_mask
                    x = self.dropout(x)
                    x = x @ (vis_weight[0] + vis_weight[2])
                    return x, vis_mask
...
def forward_llama(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor],
                  vis_weight, vis_mask=None, masktype=0):
    ...
    if masktype == 0:
        ...
        if self.training and self.gradient_checkpointing:
            h = x + torch.utils.checkpoint.checkpoint(self.attention, self.attention_norm(x), start_pos, freqs_cis, mask)
            h_norm = self.ffn_norm(h)
            h_vis, vis_mask = self.adapter_mlp(h_norm, vis_weight, vis_mask, masktype)
            out = h + torch.utils.checkpoint.checkpoint(self.feed_forward, h_norm) + h_vis * self.s
        else:
            h = x + self.attention(self.attention_norm(x), start_pos, freqs_cis, mask)
            h_norm = self.ffn_norm(h)
            h_vis, vis_mask = self.adapter_mlp(h_norm, vis_weight, vis_mask, masktype)
            out = h + self.drop_path(self.feed_forward(h_norm) + h_vis * self.s)
        if masktype == 0:
            return out
        else:
            return out, vis_mask
