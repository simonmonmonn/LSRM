...
class SVDProjector(nn.Module):
    ...
    def __init__(
        ...
        self.SVDvalue = nn.Parameter(torch.zeros(hidden_dim))
        self.fc2 = nn.Linear(hidden_dim, out_features)
        nn.init.xavier_uniform_(self.fc1.weight)
        nn.init.zeros_(self.fc1.bias)
        nn.init.xavier_uniform_(self.fc2.weight)
        nn.init.zeros_(self.fc2.bias)
        nn.init.constant_(self.SVDvalue, val=1)

    def forward(self, x):
        with autocast():
            x = self.fc2(self.SVDvalue * F.silu(self.fc1(x)))
        return x
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.adapter_proj = SVDProjector(1024, params.hidden_proj // 2, params.dim).float()
        self.adapter_proj_ll = SVDProjector(1024, params.hidden_proj // 2, params.dim).float()
    ...
    def forward(self, examples, labels, images=None, prefix_img=None, prefix_nonimg=None, img_indicators=None):
        ...
        image_embeds = (image_embeds + image_embeds_ll) * 0.01
        ...
        start_pos = 0
        if self.mode == 1:
            vis_mask =  torch.ones([_bsz, seqlen, self.params.emb]).half().to(image_embeds.device)
        for layer in self.layers:
            if self.mode == 1:
                h, vis_mask = layer(h, start_pos, freqs_cis, mask, [image_embeds, self.adapter_emb1, self.adapter_emb2], vis_mask, masktype=1)
            else:
    ...
    def generate(
        ...
        image_embeds_ll = image_embeds_ll * indicators.half().view(-1, 1, 1)
        image_embeds_ll = torch.cat([image_embeds_ll, torch.zeros(image_embeds_ll.size(0), self.params.emb - 256,
                                                            image_embeds_ll.size(2)).half().to(image_embeds_ll.device)], dim=1)
        ...
        for cur_pos in range(start_pos, total_len):
            ...
            with autocast():
                ...
                vis_mask =  torch.ones([_bsz, seqlen, self.params.emb]).half().to(image_embeds.device)
                for layer in self.layers:
                    h, vis_mask = layer(h, prev_pos, freqs_cis, mask_input, [image_embeds, self.adapter_emb1, self.adapter_emb2], vis_mask, masktype=1)
