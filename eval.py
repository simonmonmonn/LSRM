from lsrm.build import create_model
import torch
import argparse
from dataclasses import dataclass
from lsrm.tokenizer import Tokenizer
...
def main():
    ...
    parser.add_argument('--adapter_path', default='LSRM-seed1', type=str)
    ...
    for i in tqdm(range(total_items // bs + 1)):
        ...
        for qid in batch_qids:
            if problems[qid]['image'] is not None:
                image = Image.open(os.path.join(image_path, qid, 'image.png')).convert('RGBA')
                print("processing image:", qid)
            else:
                print("skip data:", qid, "because have no image")
                i = i + 1
                continue
            i = i + 1
    ...
    print(scores)
    import time
    with open(str(adapter_path) + '/' + str(time.time()) + '.txt', 'w') as f:
        f.write(str(scores))
